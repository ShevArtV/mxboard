...
/* Панель очереди = группа. Своя поверхность и радиус из общей шкалы, а не дефолт
   ...
   а `overflow: hidden` (он здесь ради скруглений) молча срезает то, что не влезло: список
   длиннее 60vh обрывался на последней задаче, и прокрутить к ней было нельзя — контейнер
   не переполнялся, значит его `overflow-y: auto` не давал полосы. Без сжатия контент
   перерастает контейнер, и скролл появляется. */
.mxb-queues .p-panel {
    flex: 0 0 auto;
    border: 1px solid var(--p-content-border-color, #e2e5e9);
    border-radius: var(--mxb-radius-md);
