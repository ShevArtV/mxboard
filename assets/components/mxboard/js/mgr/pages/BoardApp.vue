...
/* Панель очереди = группа. Своя поверхность и радиус из общей шкалы, а не дефолт
   ...
   `flex: 0 0 auto` обязателен. Панель — элемент flex-колонки и по умолчанию сжимается,
   а обрезка по скруглению молча съедала то, что не влезло: список длиннее 60vh обрывался
   на последней задаче, и прокрутить к ней было нельзя — контейнер не переполнялся, значит
   его `overflow-y: auto` не давал полосы. Без сжатия контент перерастает контейнер, и
   скролл появляется.

   `clip`, а не `hidden`: `hidden` делает панель scroll-контейнером, и sticky-шапка ниже
   начинает считать `top` от самой панели, которая не скроллится, — то есть уезжает вместе
   с содержимым. `clip` обрезает так же, но scroll-контейнера не создаёт, и шапка снова
   липнет к верху списка. Цена — Safari младше 16: там правило не применится, у контента
   просто не будут срезаны уголки под скругление панели. */
.mxb-queues .p-panel {
    flex: 0 0 auto;
    border: 1px solid var(--p-content-border-color, #e2e5e9);
    border-radius: var(--mxb-radius-md);
...
/* Рабочая область скроллится сама — как «Структура» и колонка канбана: фрейм
   менеджера MODX страницу не прокручивает, и таблица уезжала под нижний край
   насовсем. Здесь не max-height на всём разделе, а колонка фиксированной высоты:
   тулбар фильтров и счётчик остаются сверху, прокручивается только тело таблицы
   (scroll-height="flex"), поэтому пагинатор всегда виден внизу и до него не надо
   листать всю страницу. Плюс это переживает перенос тулбара на 2-3 ряда в узком
   окне, где фиксированный offset до таблицы уже не совпал бы. */
.mxb-ov {
    display: flex;
    flex-direction: column;
...
/* Ниже 960px менеджер MODX переключается на мобильную раскладку и там прокручивает
   страницу сам (проверено на стенде: при 960px документ скроллится, при 965px — нет).
   Держать в этом режиме жёсткую высоту вредно: раздел начинается уже на 419px от
   верха, и внутри 550px остаётся полоска тела таблицы, конкурирующая со скроллом
   страницы. Отдаём высоту содержимому — прокручивает страница. */
...
/* Таблица забирает всю оставшуюся высоту колонки. min-height: 0 обязателен: без
   него flex-элемент не сжимается ниже содержимого, и вся конструкция снова
   вырастает за нижний край экрана. */
.mxb-ov-table {
    flex: 1 1 auto;
    min-height: 0;
}
