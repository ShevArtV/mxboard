...
| GET | `/events?since=<log_id>&limit=<n>` | инкрементальная лента журнала (только менеджер) — см. [Автоматизация через Jarvis](#автоматизация-через-jarvis) |
...
Несколько сессий исполнителя под одним токеном работают параллельно **по разным задачам**. Подробный разбор схемы — [`agent-bootstrap.md`](../core/components/mxboard/docs/agent-bootstrap.md) (в поставке пакета).

## Автоматизация через Jarvis

mxBoard универсален — он не знает ни про какие боты. Но у него есть готовая точка интеграции, чтобы **внешний оркестратор** (у нас — локальный Telegram-бот Jarvis) автоматически поднимал агентские сессии на новые события доски. Есть два способа связать доску с оркестратором.

### Способ, который используем мы: поллер REST `/events`
...
while True:
    ...
    for ev in r.json()["data"]:
        cursor = ev["id"]
        if ev["action"] == "create" and ev["assignee"] == "claude-agent":
...
Вместо опроса можно повесить MODX-плагин на события `mxbOn*` (см. [Интеграция](integration.md#события)) и **толкать** уведомление на HTTP-эндпоинт оркестратора сразу при действии — без задержки опроса.

### Почему мы выбрали поллер

Оба способа рабочие — mxBoard даёт и события для webhook, и курсорную ленту для поллинга; это зрелое, законченное решение. Поллер мы предпочли **из-за простоты и потому что бот работает локально**: локальному оркестратору за NAT неудобно принимать входящие вебхуки (нужен публичный URL, туннель, обработка ретраев и подписи), а исходящий опрос `/events` не требует ничего, кроме токена и курсора, и переживает перезапуски бота — курсор просто продолжится с последнего `id`.
