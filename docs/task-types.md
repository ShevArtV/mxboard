...
| `user` | пользователь |
| `file` | файл(ы) — мультизагрузка через медиа-источник (см. [Вложения](#вложения)) |
...
2. Заполните настройки провайдера (**Настройки → область `mxboard_ai`**): [`ai_base_url`](settings.md), [`ai_api_key`](settings.md), [`ai_model`](settings.md). Пустой ключ = проверка выключена.
...
Настройка [`mxboard.ai_check_mode`](settings.md):
...
Глобальный промпт-шаблон — настройка [`mxboard.ai_check_prompt`](settings.md). Тип может переопределить его своим полем **`ai_prompt`** (пусто → берётся глобальный).
...
- хранятся в выделенном **медиа-источнике MODX** ([`mxboard.media_source`](settings.md), создаётся при установке);
- запись `mxboard_attachment` с `comment_id=0` — файл задачи, `comment_id>0` — файл сообщения;
- лимиты — настройки [`upload_max_size`](settings.md), [`upload_max_files`](settings.md), [`upload_extensions`](settings.md);
