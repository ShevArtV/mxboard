...
final class Server
{
    ...
    private function baseTools(): array
    {
        return [
            ...
            $this->tool(
                'task_delete',
                'НЕОБРАТИМО удалить карточку (автор/менеджер) — для ошибочно созданных задач.'
                . ' Вместе с карточкой уходят её комментарии, журнал и вложения (файлы стираются с диска);'
                . ' подзадачи не удаляются, а открепляются (становятся самостоятельными).'
                . ' Карточку в очереди удалять можно: если очередь была занята ею, следующая стартует автоматически.'
                . ' Факт удаления пишется в журнал доски от вашего имени, автор и исполнитель получают уведомление.'
                . ' Ненужную карточку, которую нельзя терять, вместо удаления переводите task_move в финальную стадию.'
                . self::CLOSED_NOTE,
                [
                    'task_id' => ['type' => 'string', 'description' => 'Адрес карточки: id (число) или num (напр. 2607-15).'],
                ],
    ...
    private function taskDelete(array $args): array
    {
        ...
        $taskId = (int) $task->get('id');
        // Номер и заголовок снимаем ДО удаления: после него объект уже ничего не расскажет,
        // а в ответе нужен опознавательный знак — по id агент карточку не вспомнит.
        ...
        return $this->content(
            'Карточка ' . ($num !== '' ? $num : '#' . $taskId) . ' «' . $title . '» удалена без возможности восстановления.'
        );
    }
