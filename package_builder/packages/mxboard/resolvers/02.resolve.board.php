...
// Шесть стадий, заточенных под работу с агентами: исполнитель выносит план на
// подтверждение автору, и только после одобрения приступает к работе.
...
$columns = [
    // Бэклог: новая карточка падает сюда (исполнитель уже назначен). Пустой move_roles —
    // рядовые роли назад сюда не возвращают, это может только менеджер (аварийный откат).
    ['key' => 'backlog', 'name' => 'Бэклог', 'position' => 0, 'move_roles' => '', 'is_initial' => true],
    // План: исполнитель изучил задачу и вынес план работ на подтверждение. Двигает исполнитель.
    ['key' => 'plan', 'name' => 'План', 'position' => 1, 'move_roles' => 'assignee'],
    // Подтверждение: только автор одобряет план переводом сюда. Не согласен — комментирует,
    // стадию не меняет; исполнитель правит план в колонке «План».
    ['key' => 'confirm', 'name' => 'Подтверждение', 'position' => 2, 'move_roles' => 'author'],
    // В работе: исполнитель приступил после подтверждения плана.
    ['key' => 'in_progress', 'name' => 'В работе', 'position' => 3, 'move_roles' => 'assignee'],
    // На проверке: исполнитель сдал результат — его потолок, дальше только автор.
    ['key' => 'review', 'name' => 'На проверке', 'position' => 4, 'move_roles' => 'assignee'],
    // Готово: закрывает ТОЛЬКО автор задачи (или менеджер). Исполнитель сюда не дотянется.
    ['key' => 'done', 'name' => 'Готово', 'position' => 5, 'move_roles' => 'author', 'is_final' => true],
];
