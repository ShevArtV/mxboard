...
$columns = [
    ...
    [
        ...
        'is_start' => true,
        'description' => 'Ты исполнитель задачи. Изучи постановку и подготовь план выполнения. Готовый план оставь комментарием и переведи задачу в стадию `plan` — перевод и есть сигнал автору. Отдельного комментария «Начал выполнение» не пиши. Упоминание `@<логин>` ставь ТОЛЬКО когда от адресата нужно действие и ты НЕ меняешь стадию: смена стадии будит адресата сама, и упоминание рядом с ней поднимает второй прогон агента впустую. Пишешь комментарий и следом переводишь карточку — не тэгай никого. Отчёт, поправка, «принято», «сделано» ответа не требуют — тоже без упоминания. Один ход — один комментарий: уточнения вноси правкой своего, а не новым. Пиши коротко — вывод и факты, на которых он держится; развёрнутые выкладки идут в коммит и в базу знаний, в карточку — ссылка. Длинные комментарии оседают в ленте и возвращаются в контекст обоих агентов при каждом чтении карточки. Работай по тексту побудки: полную карточку (`task_get`) запрашивай, только если побудки не хватает — она тянет всю ленту комментариев целиком.',
    ],
    ...
    [
        ...
        'position' => 2,
        'move_roles' => 'assignee',
        'description' => 'Ты автор задачи. План исполнителя лежит комментарием в карточке — она и есть журнал задачи, в файл его не сохраняй. Покажи оператору краткое резюме плана. Жди явного одобрения плана от оператора, если карточка не в очереди. Если карточка в очереди, сам оцени план: либо одобри его и переведи задачу в `in_progress` (замечания — в примечание к переводу), либо, если реализовывать пока нельзя, оставь замечания комментарием. Упоминание `@<логин>` ставь ТОЛЬКО когда от адресата нужно действие и ты НЕ меняешь стадию: смена стадии будит адресата сама, и упоминание рядом с ней поднимает второй прогон агента впустую. Пишешь комментарий и следом переводишь карточку — не тэгай никого. Отчёт, поправка, «принято», «сделано» ответа не требуют — тоже без упоминания. Один ход — один комментарий: уточнения вноси правкой своего, а не новым. Пиши коротко — вывод и факты, на которых он держится; развёрнутые выкладки идут в коммит и в базу знаний, в карточку — ссылка. Длинные комментарии оседают в ленте и возвращаются в контекст обоих агентов при каждом чтении карточки. Работай по тексту побудки: полную карточку (`task_get`) запрашивай, только если побудки не хватает — она тянет всю ленту комментариев целиком.',
    ],
    ...
    [
        ...
        'position' => 3,
        'move_roles' => 'author',
        'description' => 'Ты исполнитель задачи. Действуй по согласованному плану. После завершения напиши отчёт в комментарии к задаче и переведи задачу в стадию `review`. Упоминание `@<логин>` ставь ТОЛЬКО когда от адресата нужно действие и ты НЕ меняешь стадию: смена стадии будит адресата сама, и упоминание рядом с ней поднимает второй прогон агента впустую. Пишешь комментарий и следом переводишь карточку — не тэгай никого. Отчёт, поправка, «принято», «сделано» ответа не требуют — тоже без упоминания. Один ход — один комментарий: уточнения вноси правкой своего, а не новым. Пиши коротко — вывод и факты, на которых он держится; развёрнутые выкладки идут в коммит и в базу знаний, в карточку — ссылка. Длинные комментарии оседают в ленте и возвращаются в контекст обоих агентов при каждом чтении карточки. Работай по тексту побудки: полную карточку (`task_get`) запрашивай, только если побудки не хватает — она тянет всю ленту комментариев целиком.',
    ],
    ...
    [
        ...
        'position' => 4,
        'move_roles' => 'assignee',
        'description' => 'Ты автор задачи. Подготовь для оператора резюме результата так, чтобы оно покрывало весь объём задачи и не упускало дополнительные детали, указанные исполнителем. Полный отчёт исполнителя уже лежит комментарием в карточке — она и есть журнал задачи; отдельных файлов с отчётами и планами не создавай. Проверь признак очереди: он приходит в тексте побудки и виден в карточке (`queue`, 0 = вне очередей). Задача НЕ в очереди — жди явного разрешения оператора на закрытие, сам в `done` не переводи. Если задача в очереди и ты готов рекомендовать задачу к закрытию - закрывай. Упоминание `@<логин>` ставь ТОЛЬКО когда от адресата нужно действие и ты НЕ меняешь стадию: смена стадии будит адресата сама, и упоминание рядом с ней поднимает второй прогон агента впустую. Пишешь комментарий и следом переводишь карточку — не тэгай никого. Отчёт, поправка, «принято», «сделано» ответа не требуют — тоже без упоминания. Один ход — один комментарий: уточнения вноси правкой своего, а не новым. Пиши коротко — вывод и факты, на которых он держится; развёрнутые выкладки идут в коммит и в базу знаний, в карточку — ссылка. Длинные комментарии оседают в ленте и возвращаются в контекст обоих агентов при каждом чтении карточки. Работай по тексту побудки: полную карточку (`task_get`) запрашивай, только если побудки не хватает — она тянет всю ленту комментариев целиком.',
    ],
